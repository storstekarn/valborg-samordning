...
const EVENT_DATE_ORDER: EventDate[] = ['fore', 'valborg', '1maj']
...
function groupByArea(tasks: Task[]): Record<string, Task[]> {
...
export default function TaskBoard({ initialTasks }: Props) {
  ...
  const tasksByDate = EVENT_DATE_ORDER.reduce<Record<string, Task[]>>((acc, date) => {
    acc[date] = tasks.filter((t) => t.event_date === date)
    return acc
  }, {})
  ...
  return (
    <div className="space-y-10">
      ...
      {EVENT_DATE_ORDER.map((date) => {
        const dateTasks = tasksByDate[date]
        if (!dateTasks || dateTasks.length === 0) return null
        const grouped = groupByArea(dateTasks)
                ...
                return (
                  <div key={area}>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded border ${areaColor}`}>
                        {area}
                      </span>
                    </div>
                    ...
                    <div className="space-y-2">
                      {sorted.map((task) => (
                        ...
                        >
                          {/* Tid */}
                          <div className="shrink-0 w-20 text-xs text-zinc-500 pt-0.5 font-mono">
                            {task.start_time ? (
                              <>
                                {formatTime(task.start_time)}
                                {task.end_time && (
                                  <span className="block">{formatTime(task.end_time)}</span>
                                )}
                              </>
                            ) : (
